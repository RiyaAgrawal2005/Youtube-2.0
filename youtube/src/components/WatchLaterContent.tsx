...
export default function WatchLaterContent() {
  ...
  if (loading) return <div className="text-gray-700 dark:text-gray-300">Loading watch later...</div>;
  ...
  if (!user) {
    return (
      <div className="text-center py-12 text-gray-700 dark:text-gray-300">
        ...
        <h2 className="text-xl font-semibold mb-2">Save videos for later</h2>
        <p>Sign in to access your Watch later playlist.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-gray-600 dark:text-gray-400">{watchLater.length} videos</p>
        <Button className="flex items-center gap-2 bg-gray-200 dark:bg-gray-700 text-black dark:text-white">
          <Play className="w-4 h-4" />
          Play all
        </Button>
      </div>
      ...
      <div className="space-y-4">
        {watchLater.map((item) => (
          <div
            key={item._id}
            className="flex gap-4 group bg-white dark:bg-gray-800 rounded-lg p-2"
          >
            <Link href={`/watch/${item.videoid._id}`} className="flex-shrink-0">
              <div className="relative w-40 aspect-video bg-gray-100 dark:bg-gray-700 rounded overflow-hidden">
                <video
                  src={`${process.env.BACKEND_URL}/${item.videoid?.filepath}`}
                  className="object-cover group-hover:scale-105 transition-transform duration-200"
                />
            ...
            <div className="flex-1 min-w-0 text-gray-800 dark:text-gray-200">
              ...
              <p className="text-sm">{item.videoid.videochanel}</p>
              <p className="text-sm">
                {item.videoid.views.toLocaleString()} views •{" "}
                {formatDistanceToNow(new Date(item.videoid.createdAt))} ago
              </p>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300">
